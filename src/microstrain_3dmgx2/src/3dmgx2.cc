...
//Code to encode a floating point number to send to the IMU
static void encode_float(float number, uint8_t* converted){
  //uint8_t converted[4];
  
  *(converted) = *((unsigned char*)(&number)+3);
  *(converted+1) = *((unsigned char*)(&number)+2);
  *(converted+2) = *((unsigned char*)(&number)+1);
  *(converted+3) = *((unsigned char*)(&number));
...
// Set gyro bias
void
microstrain_3dmgx2_imu::IMU::setGyroBias(double &bias_x, double &bias_y, double &bias_z){

  uint8_t cmd[15];
  uint8_t rep[19];

  cmd[0] = CMD_WRITE_GYRO_BIAS;
  cmd[1] = 0x12;//Confirms user intent
  cmd[2] = 0xA5;//Confirms user intent

  //(unsigned short*)(&cmd[3]) = encode_float(((float)bias_x));
  encode_float(((float)bias_x), &cmd[3]);
  encode_float(((float)bias_y), &cmd[7]);
  encode_float(((float)bias_z), &cmd[11]);
  //(cmd[7]) = encode_float(((float)bias_y));
  //(cmd[11]) = encode_float(((float)bias_z));

  transact(cmd, sizeof(cmd), rep, sizeof(rep), 30000);
  
  double check_x, check_y, check_z;
  check_x = extract_float(rep+1);
  check_y = extract_float(rep+5);
  check_z = extract_float(rep+9);
